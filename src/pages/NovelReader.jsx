...
const getInitialSettings = () => {
  ...
  return {}; // Return empty object if nothing is saved or if there's an error
};
...
function NovelReader() {
  ...
  // Initialize settings state from localStorage or use defaults
  ...
  // Update resetSettings to also clear localStorage
  const resetSettings = useCallback(() => {
    setCurrentFont('font-inter');
    setCurrentTextSize(16);
    setCurrentLineHeight(24);
    setCurrentContrast('normal');
    localStorage.removeItem('novelReaderSettings'); // Remove saved settings
  }, []);
  ...
  const currentChapterTitle = novelData.chapters[chapterNumber]?.chapter_title || `Chapter ${getChapterNumber(chapterNumber)}`;
      ...
      <main className="p-4 max-w-4xl mx-auto flex-grow">
        {loadingChapter ? (
          ...
          <>
            ...
            <h2 className="text-2xl sm:text-3xl font-semibold text-gray-200 mb-6 text-center">{currentChapterTitle}</h2>
            
            <div className={`text-lg leading-relaxed mb-8 prose prose-invert max-w-none ${currentFont}`} style={contentStyle}>
            ...
            {/* --- END OF UPDATED NAVIGATION SECTION --- */}
          </>
        )}
      </main>
      <Footer />
