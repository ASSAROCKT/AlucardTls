...
  const { novelSlug, chapterNumber } = useParams(); // chapterNumber is "chapter-X"
...
    </div>
  );
  if (!novelData || !novelData.chapters[chapterNumber]) return <div className="min-h-screen flex items-center justify-center bg-gray-950 text-gray-400"><div className="text-xl font-semibold">Novel or chapter data could not be loaded.</div></div>;
  
  const currentChapterDetails = novelData.chapters[chapterNumber];
  const displayChapterNum = currentChapterDetails?.display_chapter;
  const displayVolumeNum = currentChapterDetails?.volume;
  const chapterSpecificTitle = currentChapterDetails?.title;
  const fullChapterTitle = `Volume ${displayVolumeNum} Chapter ${displayChapterNum}${chapterSpecificTitle ? ` - ${chapterSpecificTitle}` : ''}`;
  
...
                clearInterval(removePornInterval);
              }
            }
...
      {showChapterMenu && (
        <ChapterListMenu chapters={novelData.chapters} currentChapterKey={chapterNumber} novelSlug={novelSlug} onClose={() => setShowChapterMenu(false)} />
      )}
      {showSettingsMenu && (
...
                  Vol. {displayVolumeNum} Ch. {displayChapterNum}
                </li>
              </ol>
            </nav>
            
