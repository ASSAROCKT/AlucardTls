...
import { slugify } from '../utils/slugify'; // Assuming slugify is correctly defined here
...
const formatTimestampToTimeAgo = (timestamp) => {
  ...
  const now = Date.now();
  const then = parseInt(timestamp); // Timestamps are in milliseconds
...
const TruncatedDescription = ({ text, maxLength = 300, maxLines = 4 }) => {
  ...
  useEffect(() => {
    if (textRef.current) {
      // A simple check for overflow based on content length
...
function NovelSeriesPage() {
  ...
  // MODIFIED: This useEffect now performs a two-step fetch.
  useEffect(() => {
    if (!novelSlug) {
    ...
    const fetchNovelData = async () => {
      try {
        ...
        // --- STEP 1: Fetch the main list of all novels ---
        ...
        // --- STEP 2: Find the current novel in the list by matching the slug ---
        ...
        // --- STEP 3: Use the URL from the found novel object to fetch its specific data ---
  ...
  const filteredAndSortedChapterKeys = useMemo(() => {
    ...
    const filtered = keys.filter(chapterKey => {
      ...
      const chapterNum = chapterKey.replace('chapter-', '').toLowerCase();
      const search = searchTerm.toLowerCase();
      return chapterTitle.includes(search) || chapterNum.includes(search);
    });

    return filtered.sort((a, b) => {
      const chapterA = novelData.chapters[a];
      const chapterB = novelData.chapters[b];
      ...
      if (sortOrder === 'latest') {
        return (volB - volA) || (numB - numA);
      } else {
        return (volA - volB) || (numA - numB);
      }
    });
  ...
  const firstChapterKey = chapterKeys.length > 0 ? [...chapterKeys].sort((a, b) => parseFloat(a.replace('chapter-', '')) - parseFloat(b.replace('chapter-', '')))[0] : null;
  const lastChapterKey = chapterKeys.length > 0 ? [...chapterKeys].sort((a, b) => parseFloat(b.replace('chapter-', '')) - parseFloat(a.replace('chapter-', '')))[0] : null;
                ...
                return (
                  <Link key={chapterKey} to={`/novel/${novelSlug}/${chapterKey}`} className={`flex justify-between items-center py-3 px-2 hover:bg-gray-800 transition duration-150 ease-in-out rounded-md md:py-3.5 md:px-2.5 text-white`}>
                    <div className="truncate text-sm font-medium md:text-base">
                      ...
                      <span>Ch. {chapterKey.replace('chapter-', '')}</span>
