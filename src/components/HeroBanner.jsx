...
      try {
        setLoading(true);
        // Corrected URL from the provided code snippet
        const response = await fetch('https://raw.githubusercontent.com/ASSAROCKT/aphroditescans/refs/heads/main/Alucardbanner.json');
        if (!response.ok) {
...
    >
      {/* Background Images for PC and Mobile */}
      
      {/* PC Cover - Hidden on small screens, visible on medium and up */}
      <img
        src={currentNovel.pcCover || currentNovel.mobileCover} // Fallback to mobileCover if pcCover is missing
        alt={`${currentNovel.title} Banner`}
        className="hidden md:block absolute inset-0 w-full h-full object-cover object-top transition-transform duration-1000 ease-in-out transform scale-100 group-hover:scale-105"
        onError={(e) => {
...
      {/* Mobile Cover - Visible on small screens, hidden on medium and up */}
      <img
        src={currentNovel.mobileCover || currentNovel.pcCover} // Fallback to pcCover if mobileCover is missing
        alt={`${currentNovel.title} Banner`}
        className="block md:hidden absolute inset-0 w-full h-full object-cover object-top transition-transform duration-1000 ease-in-out transform scale-100 group-hover:scale-105"
        onError={(e) => {
          e.target.onerror = null; // Prevent infinite loop if placeholder fails
          e.target.src = "https://placehold.co/1200x700/333333/FFFFFF?text=Image+Not+Found";
        }}
